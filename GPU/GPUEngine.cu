...
#include "GPUComputeFast.h"

// ---------------------------------------------------------------------------------------
// This Code Change by Alek76 30.10.2023 
// ---------------------------------------------------------------------------------------
// ---------------------------------------------------------------------------------------

__global__ void comp_keys(uint64_t *Tkangaroos, uint64_t DPmodule, uint32_t hop_modulo, uint32_t maxFound, uint32_t *found) {

  int xPtr = (blockIdx.x*blockDim.x*GPU_GRP_SIZE) * 12; // px[4] , py[4] , tk[4]
    
  ComputeKeys(Tkangaroos + xPtr, DPmodule, hop_modulo, maxFound, found);
...
GPUEngine::GPUEngine(int nbThreadGroup, int nbThreadPerGroup, int gpuId, uint32_t maxFound, bool rekey, uint32_t pow2w, uint64_t totalK, uint32_t hop_modulo, int power, int fixedDP) { 
  ...
  //hop_modulo = (uint32_t)NB_JUMP;// Jump Table size: 128
  ...
  printf("GPUEngine: Total kangaroos and their close relatives %llu ;-)\n", (unsigned long long)totalK);
  ...
  // Input kangaroos
  kangarooSize = (uint32_t)nbThread * GPU_GRP_SIZE * 96;
  ...
  kangarooSizePinned = (uint32_t)nbThreadPerGroup * GPU_GRP_SIZE * 96;
...
GPUEngine::~GPUEngine() {
  ...
  cudaFree(inputKangaroo);// points and keys
...
bool GPUEngine::SetKangaroos(Point *p, Int *d, Int *distance, Int *px, Int *py) {
  ...
  for(int b = 0; b < nbBlock; b++) {
    for(int g = 0; g < GPU_GRP_SIZE; g++) {
      for(int t = 0; t < nbThreadPerGroup; t++) {
        ...
        // X
        inputKangarooPinned[g * strideSize + t + 0 * nbThreadPerGroup] = p[idx].x.bits64[0];
        inputKangarooPinned[g * strideSize + t + 1 * nbThreadPerGroup] = p[idx].x.bits64[1];
        inputKangarooPinned[g * strideSize + t + 2 * nbThreadPerGroup] = p[idx].x.bits64[2];
        inputKangarooPinned[g * strideSize + t + 3 * nbThreadPerGroup] = p[idx].x.bits64[3];

        // Y
        inputKangarooPinned[g * strideSize + t + 4 * nbThreadPerGroup] = p[idx].y.bits64[0];
        inputKangarooPinned[g * strideSize + t + 5 * nbThreadPerGroup] = p[idx].y.bits64[1];
        inputKangarooPinned[g * strideSize + t + 6 * nbThreadPerGroup] = p[idx].y.bits64[2];
        inputKangarooPinned[g * strideSize + t + 7 * nbThreadPerGroup] = p[idx].y.bits64[3];
        ...
        idx++;
      }// end nbThreadPerGroup
    
	}// end GPU_GRP_SIZE
	
	uint32_t offset = (uint32_t)(b * blockSize);
...
// add function
bool GPUEngine::GetKangaroos(Point *p, Int *d) {

  if(inputKangarooPinned==NULL ) {
    printf("\nGPUEngine: GetKangaroos: Cannot retreive kangaroos, mem has been freed\n");
    return false;
  }
  ...
  for(int b = 0; b < nbBlock; b++) {
    ...
    for(int g = 0; g < GPU_GRP_SIZE; g++) {
      ...
      for(int t = 0; t < nbThreadPerGroup; t++) {
        ...
        // X
        p[idx].x.bits64[0] = inputKangarooPinned[g * strideSize + t + 0 * nbThreadPerGroup];
        p[idx].x.bits64[1] = inputKangarooPinned[g * strideSize + t + 1 * nbThreadPerGroup];
        p[idx].x.bits64[2] = inputKangarooPinned[g * strideSize + t + 2 * nbThreadPerGroup];
        p[idx].x.bits64[3] = inputKangarooPinned[g * strideSize + t + 3 * nbThreadPerGroup];
        
        // Y
        p[idx].y.bits64[0] = inputKangarooPinned[g * strideSize + t + 4 * nbThreadPerGroup];
        p[idx].y.bits64[1] = inputKangarooPinned[g * strideSize + t + 5 * nbThreadPerGroup];
        p[idx].y.bits64[2] = inputKangarooPinned[g * strideSize + t + 6 * nbThreadPerGroup];
        p[idx].y.bits64[3] = inputKangarooPinned[g * strideSize + t + 7 * nbThreadPerGroup];
...
bool GPUEngine::Launch(std::vector<ITEM> &prefixFound, bool spinWait) {
  ...
  // Check copy outputPrefix
  cudaError_t error_cpy = cudaGetLastError();
  if(error_cpy != cudaSuccess) {
    printf("GPUEngine: Launch: cudaMemcpy outputPrefix: %s\n",cudaGetErrorString(err));
    return false;
  }
  //
